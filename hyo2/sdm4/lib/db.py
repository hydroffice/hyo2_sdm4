...
from hyo2.sdm4 import __version__ as version
from hyo2.sdm4 import name as name
from hyo2.sdm4.lib.export import ExportDb
...
class MonitorDb:
    """Class that provides an interface to a SQLite db with Survey Monitor data"""
            ...
            with self.conn:
                ...
                if ret[0] == 0:
                    ...
                    self.conn.execute("""
                                      ...
                                      """,
                                      (1, "%s v.%s" % (name, version), datetime.now(tz=timezone.utc),))
                # noinspection SqlNoDataSourceInspection
                self.conn.execute("""
    ...
    def point_by_id(self, pid):
        ...
        with self.conn:
            try:
                # noinspection SqlNoDataSourceInspection
                row = self.conn.execute("SELECT * FROM data WHERE id=?", (pid,)).fetchone()
                ...
                logger.error("while retrieving %s point, %s: %s" % (pid, type(e), e))
                return None

        return timestamp, long, lat, tss, draft, avg_depth

    def delete_point_by_id(self, pid):
        """Delete all the entries related to a point id"""

        with self.conn:
            try:
                # noinspection SqlNoDataSourceInspection
                self.conn.execute("""DELETE FROM data WHERE id=?""", (pid,))
                # logger.info("deleted %s id entry from data" % id)

            except sqlite3.Error as e:
                logger.error("during deletion from ssp_pk, %s: %s" % (type(e), e))
                raise RuntimeError("unable to delete point with id: %s" % pid)
